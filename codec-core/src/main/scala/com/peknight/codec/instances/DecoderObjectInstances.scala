...
trait DecoderObjectInstances extends DecoderObjectInstances1:
  given decodeObjectO[F[_], S, O](using applicative: Applicative[F], objectType: ObjectType.Aux[S, O], show: Show[S])
  ...
  given decodeMapO[F[_], S, K, V](
    using Applicative[F], Decoder[F, String, K], Decoder[F, Cursor[S], V], ObjectType[S], Show[S]
