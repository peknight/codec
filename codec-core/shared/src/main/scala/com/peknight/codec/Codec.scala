...
object Codec extends CodecDerivation:
  ...
  def mapOption[F[_]: Applicative, S, T: Show, A: ClassTag](encode: A => S)(decode: T => Option[A]): Codec[F, S, T, A] =
    applicative[F, S, T, A](encode)(t => decode(t).toRight(WrongClassTag[A].value(t)))

  def mapTry[F[_]: Applicative, S, T: Show, A: ClassTag](encode: A => S)(decode: T => Try[A]): Codec[F, S, T, A] =
    applicative[F, S, T, A](encode)(t => decode(t).toEither.left.map(e => ParsingTypeError[A](e).value(t)))

  def parse[F[_]: Applicative, S, T: Show, A: ClassTag](encode: A => S)(decode: T => A): Codec[F, S, T, A] =
  ...
  def cursor[F[_]: Applicative, S: Show, A](encode0: A => F[S])(decode0: SuccessCursor[S] => F[Either[DecodingFailure, A]])
  ...
  def cursorApplicative[F[_]: Applicative, S: Show, A](encode: A => S)
                                                      (decode: SuccessCursor[S] => Either[DecodingFailure, A])
  : Codec[F, S, Cursor[S], A] =
    cursor[F, S, A](a => encode(a).pure[F])(t => decode(t).pure[F])

  def cursorMap[F[_]: Applicative, S: Show, A](encode: A => S)(decode: SuccessCursor[S] => A)
  : Codec[F, S, Cursor[S], A] =
    cursorApplicative[F, S, A](encode)(t => decode(t).asRight[DecodingFailure])

  def cursorIdentity[F[_]: Applicative, S: Show]: Codec[F, S ,Cursor[S], S] =
    cursorMap[F, S, S](Predef.identity)(_.value)

  def cursorConst[F[_]: Applicative, S: Show, A](encode: S)(decode: A): Codec[F, S, Cursor[S], A] =
    cursorMap[F, S, A](_ => encode)(_ => decode)

  def cursorValue[F[_]: Applicative, S: Show, A](encode: A => F[S])(decode: S => F[Either[DecodingFailure, A]])
  : Codec[F, S, Cursor[S], A] =
    cursor[F, S, A](encode)(t => decode(t.value).map(_.left.map(_.cursor(t))))

  def cursorValueApplicative[F[_]: Applicative, S: Show, A](encode: A => S)(decode: S => Either[DecodingFailure, A])
  : Codec[F, S, Cursor[S], A] =
    cursorApplicative(encode)(t => decode(t.value).left.map(_.cursor(t)))

  def cursorValueMap[F[_]: Applicative, S: Show, A](encode: A => S)(decode: S => A): Codec[F, S, Cursor[S], A] =
  ...
  def codecN[F[_], S, A](using Encoder[F, Number, A], Decoder[F, Number, A])
                        (using Applicative[F], NumberType[S], Show[S], ClassTag[A]): Codec[F, S, Cursor[S], A] =
  ...
  def codecNS[F[_], S, A](using Encoder[F, Number, A], Decoder[F, Number, A])
                         (using Applicative[F], NumberType[S], StringType[S], Show[S], ClassTag[A]): Codec[F, S, Cursor[S], A] =
