...
trait DecoderInstances:
  given derivedDecoder[F[_], T, E, A](using
    ...
    LowPriority(DecoderInstances.derived[F, T, E, A])
...
object DecoderInstances extends DecoderInstances:
  def derived[F[_], T, E, A](using configuration: DecoderConfiguration)(using
    ...
    instances.derive(
      inst ?=> derivedProduct[F, T, E, A](configuration, decodeObject, failure, inst),
      inst ?=> derivedSum[F, T, E, A](configuration, decodeObject, failure, stringOptionDecoder, inst)
    )
  ...
  private[this] def derivedProduct[F[_]: Applicative, T, E, A](
    ...
    new Decoder[F, T, E, A]:
      def decode(t: T): F[Either[E, A]] = decodeProductEither(t, configuration, decodeObject, failure, instances)
      def decodeAccumulating(t: T): F[ValidatedNel[E, A]] =
        decodeProductValidatedNel(t, configuration, decodeObject, failure, instances)
  end derivedProduct
  ...
  private[this] def derivedSum[F[_] : Monad, T, E, A](
    ...
    new SumDecoder[F, T, E, A]:
      ...
      def decode(t: T): F[Either[E, A]] =
        decodeSumEither(t, configuration0, decodeObject, failure, stringOptionDecoder, instances)
      def decodeAccumulating(t: T): F[ValidatedNel[E, A]] =
        decodeSumValidatedNel(t, configuration0, decodeObject, failure, stringOptionDecoder, instances)
  end derivedSum
  ...
  private[this] def decodeProduct[F[_]: Applicative, G[_]: Applicative, T, E, A](
    ...
    if decodeObject.isObject(t) then
      if configuration.strictDecoding then
        ...
        val unexpectedFields = decodeObject.keys(t).map(_.filterNot(expectedFieldsSet)).getOrElse(Nil)
        ...
        else handleDecodeProduct(t, configuration, decodeObject, decode.asInstanceOf, isRight, instances)
      else handleDecodeProduct(t, configuration, decodeObject, decode.asInstanceOf, isRight, instances)
  ...
  private[this] def handleDecodeProduct[F[_]: Applicative, G[_]: Applicative, T, E, A](
    ...
    instances.constructWithLabelDefault[[X] =>> F[G[X]]] {
      [X] => (decoder: Decoder[F, T, E, X], label: String, defaultOpt: Option[X]) =>
        ...
        val current = decodeObject.downField(t, key)
        decode(decoder)(current).map(result => defaultOpt
          .filter(_ => configuration.useDefaults &&
            (!decodeObject.decodeContains(t, key) || (!isRight(result) && decodeObject.isNull(current)))
          )
  ...
  private[this] def decodeSum[F[_]: Monad, G[_], T, E, A](
    ...
    configuration.discriminator match
      case Some(discriminator) =>
        val current = decodeObject.downField(t, discriminator)
        stringOptionDecoder.decode(current).flatMap {
          case Right(Some(sumTypeName)) =>
            handleDecodeSum[F, G, T, E, A](t, sumTypeName, configuration, decodeObject, failure, asLeft,
